...
\section{Mô hình ERD do nhóm cung cấp}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.76\textheight,keepaspectratio]{figures/erd_tong_quan.png}
\caption{ERD tổng quan do nhóm cung cấp}
\label{fig:erd}
\end{figure}

Hình trên là sơ đồ ERD tổng quan mà nhóm đã thực hiện. Các entity bao phủ role/user, farm/zone, mission/UAV/gateway, sensor node/sensor data, threshold, alert và notification.
...
Các ảnh schema chi tiết của nhóm được đưa vào báo cáo ở Phụ lục. Các bảng và cột dưới đây được chép lại theo sơ đồ đã cung cấp.
...
Các sơ đồ nhóm đã gửi được tích hợp nguyên bản trong Phụ lục. Chúng được chia theo các cụm: user/farm/zone; sensor/threshold; UAV/mission; collection/alert.
